...
#define EAF_HW_JPEG_ENABLED 1
#include "driver/jpeg_decode.h"
#else
#define EAF_HW_JPEG_ENABLED 0
#endif

static const char *TAG = "eaf_decoder";

/**********************
 *  STATIC VARIABLES
 **********************/
static block_decoder_t eaf_decoder[EAF_ENCODING_MAX] = {0};

#if EAF_HW_JPEG_ENABLED
 ...
 */
#define EAF_ALIGN_UP(num, align) (((num) + ((align) - 1)) & ~((align) - 1))

/**
 * @brief Get cache alignment for PSRAM
 */
static inline size_t eaf_get_psram_cache_align(void)
{
 ...
 */
static inline void *eaf_malloc_prefer_psram(size_t size)
{
    void *ptr = NULL;
 ...
 */
static inline void *eaf_malloc_aligned_prefer_psram(size_t size, size_t alignment, size_t *allocated_size)
{
    void *ptr = NULL;
    ...
    if (alignment == 0) {
        alignment = eaf_get_psram_cache_align();
    }

    /* Align size up to alignment boundary */
    aligned_size = EAF_ALIGN_UP(size, alignment);
 ...
 * Tries to allocate from PSRAM first, falls back to internal RAM if unavailable.
 */
static inline void *eaf_calloc_prefer_psram(size_t num, size_t size)
{
    void *ptr = NULL;
 ...
 * @return Pointer to allocated memory, or NULL on failure
 */
static inline void *eaf_calloc_aligned_prefer_psram(size_t num, size_t size, size_t alignment, size_t *allocated_size)
{
    size_t total_size = num * size;
    void *ptr = eaf_malloc_aligned_prefer_psram(total_size, alignment, allocated_size);
    if (ptr != NULL) {
...
static uint32_t eaf_format_calc_checksum(const uint8_t *data, uint32_t length)
{
    uint32_t checksum = 0;
...
{
    HuffmanNode *node = (HuffmanNode *)eaf_calloc_prefer_psram(1, sizeof(HuffmanNode));
    return node;
}
                                     ...
                                     uint8_t *decoded_data, size_t *decoded_len)
{
    ...
    while (dict_pos < dict_len) {
        if (dict_pos + 1 >= dict_len) {
            ESP_LOGE(TAG, "Dictionary truncated at position %d", (int)dict_pos);
        ...
        if (dict_pos + code_byte_len > dict_len) {
            ESP_LOGE(TAG, "Dictionary overflow at position %d", (int)dict_pos);
            huffman_tree_free(root);
            return ESP_FAIL;
        }
...
eaf_format_t lv_eaf_header_parse(const uint8_t *file_data, size_t file_size, eaf_header_t *header)
{
    ...
    if (strncmp(header->format, "_S", 2) == 0) {
        ...
        header->block_len = (uint32_t *)eaf_malloc_prefer_psram(header->blocks * sizeof(uint32_t));
        ...
        if (header->bit_depth == 24) {
            ...
            header->palette = (uint8_t *)eaf_malloc_prefer_psram(header->num_colors * 4);
        ...
        header->palette = (uint8_t *)eaf_malloc_prefer_psram(file_length + 1);
        if (header->palette == NULL) {
...
void lv_eaf_free_header(eaf_header_t *header)
{
    if (header->block_len != NULL) {
        free(header->block_len);
        header->block_len = NULL;
...
void lv_eaf_calculate_offsets(const eaf_header_t *header, uint32_t *offsets)
{
...
eaf_color_t lv_eaf_palette_get_color(const eaf_header_t *header, uint8_t color_index, bool swap_bytes)
{
...
static esp_err_t eaf_huffman_rle_decode(const uint8_t *input_data, size_t input_size,
                                        ...
                                        bool swap_color)
{
    ...
    uint8_t *huffman_buffer = eaf_malloc_prefer_psram(*out_size);
    ...
    esp_err_t ret = lv_eaf_huffman_decode(input_data, input_size, huffman_buffer, &huffman_out_size, swap_color);
    if (ret == ESP_OK) {
        ret = lv_eaf_rle_decode(huffman_buffer, huffman_out_size, output_buffer, out_size, swap_color);
    }
...
static esp_err_t register_decoder(eaf_encoding_t type, block_decoder_t decoder)
{
...
{
    esp_err_t ret = register_decoder(EAF_ENCODING_RLE, lv_eaf_rle_decode);
    if (ret != ESP_OK) {
        return ret;
    }

    ret = register_decoder(EAF_ENCODING_HUFFMAN, eaf_huffman_rle_decode);
    if (ret != ESP_OK) {
        return ret;
    }

    ret = register_decoder(EAF_ENCODING_JPEG, lv_eaf_jpeg_decode);
    if (ret != ESP_OK) {
        return ret;
    }

    ret = register_decoder(EAF_ENCODING_HUFFMAN_DIRECT, lv_eaf_huffman_decode);
    if (ret != ESP_OK) {
        return ret;
    }

    /* Initialize hardware JPEG decoder if available */
    ret = lv_eaf_hw_jpeg_init();
    return ret;
}

static inline uint16_t eaf_rgb565_from_bgr(const uint8_t *bgr, bool swap_color)
{
    uint16_t value = ((bgr[2] & 0xF8) << 8) | ((bgr[1] & 0xFC) << 3) | ((bgr[0] & 0xF8) >> 3);
    return swap_color ? __builtin_bswap16(value) : value;
}

static inline int eaf_get_block_rows(const eaf_header_t *header, int block_index)
{
...
esp_err_t lv_eaf_block_decode(const eaf_header_t *header, const uint8_t *frame_data,
                           int block_index, uint8_t *decode_buffer, bool swap_color)
{
    // Calculate block offsets
    uint32_t *offsets = (uint32_t *)eaf_malloc_prefer_psram(header->blocks * sizeof(uint32_t));
    if (offsets == NULL) {
        ESP_LOGE(TAG, "Failed to allocate memory for block offsets");
        return ESP_FAIL;
    }

    lv_eaf_calculate_offsets(header, offsets);
    ...
    int block_height = eaf_get_block_rows(header, block_index);
    ...
    bool is_jpeg = (encoding_type == EAF_ENCODING_JPEG);
    ...
    if (!is_jpeg) {
        work_buffer = (uint8_t *)eaf_malloc_prefer_psram(decode_capacity);
        if (!work_buffer) {
    ...
    case 24: {
        ...
        for (size_t i = 0; i < pixel_count; i++) {
            ...
            dest[dest_index++] = eaf_rgb565_from_bgr(bgr, swap_color);
        }
        break;
    }
    case 8: {
        ...
        for (size_t i = 0; i < pixel_count; i++) {
            ...
            eaf_color_t color = lv_eaf_palette_get_color(header, color_index, swap_color);
    ...
    case 4: {
        ...
        for (size_t i = 0; i < out_size && pixel_written < pixel_count; i++) {
            ...
            eaf_color_t color_high = lv_eaf_palette_get_color(header, high, swap_color);
            dest[pixel_written++] = color_high.full;
            if (pixel_written >= pixel_count) {
                break;
            }
            uint8_t low = packed & 0x0F;
            eaf_color_t color_low = lv_eaf_palette_get_color(header, low, swap_color);
            dest[pixel_written++] = color_low.full;
        }
        if (pixel_written < pixel_count) {
            ESP_LOGW(TAG, "Incomplete 4-bit block %d (%d/%d pixels)", block_index, (int)pixel_written, (int)pixel_count);
        }
        break;
    }
    default:
...
esp_err_t lv_eaf_rle_decode(const uint8_t *input_data, size_t input_size,
                         uint8_t *output_buffer, size_t *out_size,
                         bool swap_color)
{
 ...
 */
esp_err_t lv_eaf_hw_jpeg_init(void)
{
 ...
 */
esp_err_t lv_eaf_hw_jpeg_deinit(void)
{
    if (s_hw_jpeg_handle != NULL) {
 ...
 */
static esp_err_t lv_eaf_jpeg_decode_hardware(const uint8_t *jpeg_data, size_t jpeg_size,
        ...
        bool swap_color)
{
    ...
    jpeg_dec_io_t *jpeg_io = eaf_malloc_prefer_psram(sizeof(jpeg_dec_io_t));
    jpeg_dec_header_info_t *header_info = eaf_malloc_prefer_psram(sizeof(jpeg_dec_header_info_t));
    ...
    size_t cache_align = eaf_get_psram_cache_align();
...
esp_err_t lv_eaf_jpeg_decode(const uint8_t *jpeg_data, size_t jpeg_size,
                          uint8_t *decode_buffer, size_t *out_size, bool swap_color)
{
    ...
    if (s_hw_jpeg_handle != NULL) {
        esp_err_t hw_ret = lv_eaf_jpeg_decode_hardware(jpeg_data, jpeg_size,
    ...
    jpeg_dec_io_t *jpeg_io = eaf_malloc_prefer_psram(sizeof(jpeg_dec_io_t));
    jpeg_dec_header_info_t *out_info = eaf_malloc_prefer_psram(sizeof(jpeg_dec_header_info_t));
    if (!jpeg_io || !out_info) {
        if (jpeg_io) {
            free(jpeg_io);
        }
...
esp_err_t lv_eaf_huffman_decode(const uint8_t *input_data, size_t input_size,
                             uint8_t *output_buffer, size_t *out_size,
                             bool swap_color)
{
    (void)swap_color; // Unused parameter
...
esp_err_t lv_eaf_format_init(const uint8_t *data, size_t data_len, eaf_format_handle_t *ret_parser)
{
    ...
    eaf_format_ctx_t *parser = NULL;

    ESP_GOTO_ON_FALSE(data != NULL && ret_parser != NULL, ESP_ERR_INVALID_ARG, err, TAG, "invalid input");
    ESP_GOTO_ON_FALSE(data_len >= EAF_TABLE_OFFSET, ESP_ERR_INVALID_SIZE, err, TAG, "file too small");

    parser = (eaf_format_ctx_t *)eaf_calloc_prefer_psram(1, sizeof(eaf_format_ctx_t));
    ...
    ESP_GOTO_ON_FALSE((size_t)total_frames <= (SIZE_MAX / sizeof(frame_table_entry_t)), ESP_ERR_INVALID_SIZE, err, TAG, "frame count too large");

    const size_t payload_available = data_len - EAF_TABLE_OFFSET;
    const size_t table_bytes = (size_t)total_frames * sizeof(frame_table_entry_t);
    ...
    ESP_GOTO_ON_FALSE((size_t)EAF_TABLE_OFFSET + stored_len_sz <= data_len, ESP_ERR_INVALID_SIZE, err, TAG, "stored data exceeds file");

    const uint32_t calculated_chk = eaf_format_calc_checksum((uint8_t *)(data + EAF_TABLE_OFFSET), stored_len);
    ESP_GOTO_ON_FALSE(calculated_chk == stored_chk, ESP_ERR_INVALID_CRC, err, TAG, "bad full checksum");

    entries = (frame_entry_t *)eaf_malloc_prefer_psram(sizeof(frame_entry_t) * total_frames);
    ESP_GOTO_ON_FALSE(entries, ESP_ERR_NO_MEM, err, TAG, "no mem for frame entries");

    frame_table_entry_t *table = (frame_table_entry_t *)(data + EAF_TABLE_OFFSET);
    const size_t frame_region_len = stored_len_sz - table_bytes;
    const size_t frame_region_offset = EAF_TABLE_OFFSET + table_bytes;
    ...
    for (int i = 0; i < total_frames; i++) {
        ...
        ESP_GOTO_ON_FALSE(frame_size >= EAF_MAGIC_LEN, ESP_ERR_INVALID_SIZE, err, TAG, "frame %d size too small", i);
...
esp_err_t lv_eaf_format_deinit(eaf_format_handle_t handle)
{
    if (handle == NULL) {
...
int lv_eaf_format_get_total_frames(eaf_format_handle_t handle)
{
    if (handle == NULL) {
...
const uint8_t *lv_eaf_format_get_frame_data(eaf_format_handle_t handle, int index)
{
    if (handle == NULL) {
        ESP_LOGE(TAG, "Handle is invalid");
        return NULL;
...
int lv_eaf_format_get_frame_size(eaf_format_handle_t handle, int index)
{
    if (handle == NULL) {
        ESP_LOGE(TAG, "Handle is invalid");
        return -1;
