...
void lv_eaf_set_src(lv_obj_t *obj, const void *src)
{
    ...
    if (eaf_obj->eaf_handle != NULL) {
        ...
        lv_eaf_format_deinit(eaf_obj->eaf_handle);
        eaf_obj->eaf_handle = NULL;
        lv_eaf_free_header(&eaf_obj->eaf_header);
    ...
    if (LV_EAF_SRC_GET_TYPE(src) == LV_EAF_SRC_TYPE_VARIABLE) {
        ...
        uint8_t *file_data = (uint8_t *)eaf_malloc_prefer_psram(file_size);
        if (!file_data) {
...
int32_t lv_eaf_get_total_frames(lv_obj_t *obj)
{
    ...
    if (eaf_obj->eaf_handle == NULL) {
        ESP_LOGE("", "EAF resource not loaded correctly");
        return -1;
    }
...
static void lv_eaf_constructor(const lv_obj_class_t *class_p, lv_obj_t *obj)
{
    ...
    memset(&eaf_obj->eaf_header, 0, sizeof(eaf_header_t));
    eaf_obj->eaf_handle = NULL;
    eaf_obj->frame_buffer = NULL;
...
static esp_err_t load_eaf_from_data(lv_eaf_t *eaf_obj, const uint8_t *data, size_t data_size)
{
    ...
    esp_err_t ret = lv_eaf_format_init(data, data_size, &eaf_obj->eaf_handle);
    if (ret != ESP_OK) {
        LV_LOG_ERROR("Failed to initialize EAF parser");
        return ret;
    }

    eaf_obj->total_frames = lv_eaf_format_get_total_frames(eaf_obj->eaf_handle);
    eaf_obj->current_frame = 0;
    ...
    const uint8_t *frame_data = lv_eaf_format_get_frame_data(eaf_obj->eaf_handle, 0);
    if (!frame_data) {
    ...
    eaf_format_t format = lv_eaf_header_parse(frame_data, lv_eaf_format_get_frame_size(eaf_obj->eaf_handle, 0), &eaf_obj->eaf_header);
    ...
    eaf_obj->frame_buffer = (uint8_t *)eaf_malloc_aligned_prefer_psram(required_size, 0, &allocated_size);
    if (!eaf_obj->frame_buffer) {
...
static esp_err_t decode_current_frame(lv_eaf_t *eaf_obj)
{
    ...
    const uint8_t *frame_data = lv_eaf_format_get_frame_data(eaf_obj->eaf_handle, eaf_obj->current_frame);
    if (!frame_data) {
    ...
    size_t frame_size = lv_eaf_format_get_frame_size(eaf_obj->eaf_handle, eaf_obj->current_frame);
    eaf_format_t format = lv_eaf_header_parse(frame_data, frame_size, &eaf_obj->eaf_header);
    if (format != EAF_FORMAT_VALID) {
    ...
    for (int block = 0; block < eaf_obj->eaf_header.blocks; block++) {
        ...
        esp_err_t ret = lv_eaf_block_decode(&eaf_obj->eaf_header, frame_data, block, block_buffer, false);
        if (ret != ESP_OK) {
